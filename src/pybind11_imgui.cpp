...
void bind_imgui_to_py(py::module& m)
{
  ...
  py::enum_<ImGuiWindowFlags_>(m, "WindowFlags", py::arithmetic())
    ...
    .value("NoNavInputs", ImGuiWindowFlags_NoNavInputs, "No keyboard/gamepad navigation within the window")
    .value("NoNavFocus", ImGuiWindowFlags_NoNavFocus, "No focusing toward this window with keyboard/gamepad navigation (e.g. skipped by Ctrl+Tab)")
  ...
  py::enum_<ImGuiChildFlags_>(m, "ChildFlags", py::arithmetic())
    ...
    .value("AlwaysUseWindowPadding", ImGuiChildFlags_AlwaysUseWindowPadding, "Pad with style.WindowPadding even if no border are drawn (no padding by default for non-bordered child windows because it makes more sense)")
    .value("ResizeX", ImGuiChildFlags_ResizeX, "Allow resize from right border (layout direction). Enable .ini saving (unless ImGuiWindowFlags_NoSavedSettings passed to window flags)")
    .value("ResizeY", ImGuiChildFlags_ResizeY, "Allow resize from bottom border (layout direction). \"")
    .value("AutoResizeX", ImGuiChildFlags_AutoResizeX, "Enable auto-resizing width. Read \"IMPORTANT: Size measurement\" details above.")
    .value("AutoResizeY", ImGuiChildFlags_AutoResizeY, "Enable auto-resizing height. Read \"IMPORTANT: Size measurement\" details above.")
    .value("AlwaysAutoResize", ImGuiChildFlags_AlwaysAutoResize, "Combined with AutoResizeX/AutoResizeY. Always measure size even when child is hidden, always return true, always disable clipping optimization! NOT RECOMMENDED.")
    .value("FrameStyle", ImGuiChildFlags_FrameStyle, "Style the child window like a framed item: use FrameBg, FrameRounding, FrameBorderSize, FramePadding instead of ChildBg, ChildRounding, ChildBorderSize, WindowPadding.")
    .value("NavFlattened", ImGuiChildFlags_NavFlattened, "[BETA] Share focus scope, allow keyboard/gamepad navigation to cross over parent border to this child or between sibling child windows.")
  ;

  py::enum_<ImGuiItemFlags_>(m, "ItemFlags", py::arithmetic())
    .value("NONE", ImGuiItemFlags_None, "(Default)")
    .value("NoTabStop", ImGuiItemFlags_NoTabStop, "false    // Disable keyboard tabbing. This is a \"lighter\" version of ImGuiItemFlags_NoNav.")
    .value("NoNav", ImGuiItemFlags_NoNav, "false    // Disable any form of focusing (keyboard/gamepad directional navigation and SetKeyboardFocusHere() calls).")
    .value("NoNavDefaultFocus", ImGuiItemFlags_NoNavDefaultFocus, "false    // Disable item being a candidate for default focus (e.g. used by title bar items).")
    .value("ButtonRepeat", ImGuiItemFlags_ButtonRepeat, "false    // Any button-like behavior will have repeat mode enabled (based on io.KeyRepeatDelay and io.KeyRepeatRate values). Note that you can also call IsItemActive() after any button to tell if it is being held.")
    .value("AutoClosePopups", ImGuiItemFlags_AutoClosePopups, "true     // MenuItem()/Selectable() automatically close their parent popup window.")
    .value("AllowDuplicateId", ImGuiItemFlags_AllowDuplicateId, "false    // Allow submitting an item with the same identifier as an item already submitted this frame without triggering a warning tooltip if io.ConfigDebugHighlightIdConflicts is set.")
  ...
  py::enum_<ImGuiInputTextFlags_>(m, "InputTextFlags", py::arithmetic())
    ...
    .value("EnterReturnsTrue", ImGuiInputTextFlags_EnterReturnsTrue, "Return 'true' when Enter is pressed (as opposed to every time the value was modified). Consider using IsItemDeactivatedAfterEdit() instead!")
    ...
    .value("CtrlEnterForNewLine", ImGuiInputTextFlags_CtrlEnterForNewLine, "In multi-line mode, validate with Enter, add new line with Ctrl+Enter (default is opposite: validate with Ctrl+Enter, add line with Enter).")
    ...
    .value("ParseEmptyRefVal", ImGuiInputTextFlags_ParseEmptyRefVal, "InputFloat(), InputInt(), InputScalar() etc. only: parse empty string as zero value.")
    .value("DisplayEmptyRefVal", ImGuiInputTextFlags_DisplayEmptyRefVal, "InputFloat(), InputInt(), InputScalar() etc. only: when value is zero, do not display it. Generally used with ImGuiInputTextFlags_ParseEmptyRefVal.")
    ...
    .value("ElideLeft", ImGuiInputTextFlags_ElideLeft, "When text doesn't fit, elide left side to ensure right side stays visible. Useful for path/filenames. Single-line only!")
    ...
    .value("CallbackEdit", ImGuiInputTextFlags_CallbackEdit, "Callback on any edit. Note that InputText() already returns true on edit + you can always use IsItemEdited(). The callback is useful to manipulate the underlying buffer while focus is active.")
    .value("WordWrap", ImGuiInputTextFlags_WordWrap, "InputTextMultiline(): word-wrap lines that are too long.")
  ;
  ...
  py::enum_<ImGuiTreeNodeFlags_>(m, "TreeNodeFlags", py::arithmetic())
    ...
    .value("OpenOnDoubleClick", ImGuiTreeNodeFlags_OpenOnDoubleClick, "Open on double-click instead of simple click (default for multi-select unless any _OpenOnXXX behavior is set explicitly). Both behaviors may be combined.")
    .value("OpenOnArrow", ImGuiTreeNodeFlags_OpenOnArrow, "Open when clicking on the arrow part (default for multi-select unless any _OpenOnXXX behavior is set explicitly). Both behaviors may be combined.")
    ...
    .value("FramePadding", ImGuiTreeNodeFlags_FramePadding, "Use FramePadding (even for an unframed text node) to vertically align text baseline to regular widget height. Equivalent to calling AlignTextToFramePadding() before the node.")
    .value("SpanAvailWidth", ImGuiTreeNodeFlags_SpanAvailWidth, "Extend hit box to the right-most edge, even if not framed. This is not the default in order to allow adding other items on the same line without using AllowOverlap mode.")
    .value("SpanFullWidth", ImGuiTreeNodeFlags_SpanFullWidth, "Extend hit box to the left-most and right-most edges (cover the indent area).")
    .value("SpanLabelWidth", ImGuiTreeNodeFlags_SpanLabelWidth, "Narrow hit box + narrow hovering highlight, will only cover the label text.")
    .value("SpanAllColumns", ImGuiTreeNodeFlags_SpanAllColumns, "Frame will span all columns of its container table (label will still fit in current column)")
    .value("LabelSpanAllColumns", ImGuiTreeNodeFlags_LabelSpanAllColumns, "Label will span all columns of its container table")
    .value("NavLeftJumpsToParent", ImGuiTreeNodeFlags_NavLeftJumpsToParent, "Nav: left arrow moves back to parent. This is processed in TreePop() when there's an unfulfilled Left nav request remaining.")
    .value("CollapsingHeader", ImGuiTreeNodeFlags_CollapsingHeader)
    .value("DrawLinesNone", ImGuiTreeNodeFlags_DrawLinesNone, "No lines drawn")
    .value("DrawLinesFull", ImGuiTreeNodeFlags_DrawLinesFull, "Horizontal lines to child nodes. Vertical line drawn down to TreePop() position: cover full contents. Faster (for large trees).")
    .value("DrawLinesToNodes", ImGuiTreeNodeFlags_DrawLinesToNodes, "Horizontal lines to child nodes. Vertical line drawn down to bottom-most child node. Slower (for large trees).")
    .value("NavLeftJumpsBackHere", ImGuiTreeNodeFlags_NavLeftJumpsBackHere, "Renamed in 1.92.0")
    .value("SpanTextWidth", ImGuiTreeNodeFlags_SpanTextWidth, "Renamed in 1.90.7")
  ;

  py::enum_<ImGuiPopupFlags_>(m, "PopupFlags", py::arithmetic())
    .value("NONE", ImGuiPopupFlags_None)
    .value("MouseButtonLeft", ImGuiPopupFlags_MouseButtonLeft, "For BeginPopupContext*(): open on Left Mouse release. Only one button allowed!")
    .value("MouseButtonRight", ImGuiPopupFlags_MouseButtonRight, "For BeginPopupContext*(): open on Right Mouse release. Only one button allowed! (default)")
    .value("MouseButtonMiddle", ImGuiPopupFlags_MouseButtonMiddle, "For BeginPopupContext*(): open on Middle Mouse release. Only one button allowed!")
    .value("NoReopen", ImGuiPopupFlags_NoReopen, "For OpenPopup*(), BeginPopupContext*(): don't reopen same popup if already open (won't reposition, won't reinitialize navigation)")
    ...
    .value("MouseButtonShift_", ImGuiPopupFlags_MouseButtonShift_, "[Internal]")
    .value("MouseButtonMask_", ImGuiPopupFlags_MouseButtonMask_, "[Internal]")
    .value("InvalidMask_", ImGuiPopupFlags_InvalidMask_, "[Internal] Reserve legacy bits 0-1 to detect incorrectly passing 1 or 2 to the function.")
  ;

  py::enum_<ImGuiSelectableFlags_>(m, "SelectableFlags", py::arithmetic())
    .value("NONE", ImGuiSelectableFlags_None)
    .value("NoAutoClosePopups", ImGuiSelectableFlags_NoAutoClosePopups, "Clicking this doesn't close parent popup window (overrides ImGuiItemFlags_AutoClosePopups)")
    .value("SpanAllColumns", ImGuiSelectableFlags_SpanAllColumns, "Frame will span all columns of its container table (text will still fit in current column)")
    ...
    .value("SelectOnNav", ImGuiSelectableFlags_SelectOnNav, "Auto-select when moved into, unless Ctrl is held. Automatic when in a BeginMultiSelect() block.")
    .value("DontClosePopups", ImGuiSelectableFlags_DontClosePopups, "Renamed in 1.91.0")
  ;
  ...
  py::enum_<ImGuiComboFlags_>(m, "ComboFlags", py::arithmetic())
    ...
    .value("WidthFitPreview", ImGuiComboFlags_WidthFitPreview, "Width dynamically calculated from preview contents")
  ...
  py::enum_<ImGuiTabBarFlags_>(m, "TabBarFlags", py::arithmetic())
    ...
    .value("NoCloseWithMiddleMouseButton", ImGuiTabBarFlags_NoCloseWithMiddleMouseButton, "Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You may handle this behavior manually on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open = false.")
    ...
    .value("DrawSelectedOverline", ImGuiTabBarFlags_DrawSelectedOverline, "Draw selected overline markers over selected tab")
    .value("FittingPolicyMixed", ImGuiTabBarFlags_FittingPolicyMixed, "Shrink down tabs when they don't fit, until width is style.TabMinWidthShrink, then enable scrolling buttons.")
    .value("FittingPolicyShrink", ImGuiTabBarFlags_FittingPolicyShrink, "Shrink down tabs when they don't fit")
    .value("FittingPolicyScroll", ImGuiTabBarFlags_FittingPolicyScroll, "Enable scrolling buttons when tabs don't fit")
    ...
    .value("FittingPolicyResizeDown", ImGuiTabBarFlags_FittingPolicyResizeDown, "Renamed in 1.92.2")
  ;

  py::enum_<ImGuiTabItemFlags_>(m, "TabItemFlags", py::arithmetic())
    .value("NONE", ImGuiTabItemFlags_None)
    .value("UnsavedDocument", ImGuiTabItemFlags_UnsavedDocument, "Display a dot next to the title + set ImGuiTabItemFlags_NoAssumedClosure.")
    ...
    .value("NoCloseWithMiddleMouseButton", ImGuiTabItemFlags_NoCloseWithMiddleMouseButton, "Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You may handle this behavior manually on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open = false.")
    .value("NoPushId", ImGuiTabItemFlags_NoPushId, "Don't call PushID()/PopID() on BeginTabItem()/EndTabItem()")
    ...
    .value("NoAssumedClosure", ImGuiTabItemFlags_NoAssumedClosure, "Tab is selected when trying to close + closure is not immediately assumed (will wait for user to stop submitting the tab). Otherwise closure is assumed when pressing the X, so if you keep submitting the tab may reappear at end of tab bar.")
  ;
  ...
  py::enum_<ImGuiHoveredFlags_>(m, "HoveredFlags", py::arithmetic())
    ...
    .value("NoNavOverride", ImGuiHoveredFlags_NoNavOverride, "IsItemHovered() only: Disable using keyboard/gamepad navigation state when active, always query mouse")
  ...
  py::enum_<ImGuiDockNodeFlags_>(m, "DockNodeFlags", py::arithmetic())
    ...
    .value("KeepAliveOnly", ImGuiDockNodeFlags_KeepAliveOnly, "// Don't display the dockspace node but keep it alive. Windows docked into this dockspace node won't be undocked.")
    .value("NoDockingOverCentralNode", ImGuiDockNodeFlags_NoDockingOverCentralNode, "// Disable docking over the Central Node, which will be always kept empty.")
    .value("PassthruCentralNode", ImGuiDockNodeFlags_PassthruCentralNode, "// Enable passthru dockspace: 1) DockSpace() will render a ImGuiCol_WindowBg background covering everything excepted the Central Node when empty. Meaning the host window should probably use SetNextWindowBgAlpha(0.0f) prior to Begin() when using this. 2) When Central Node is empty: let inputs pass-through + won't display a DockingEmptyBg background. See demo for details.")
    .value("NoDockingSplit", ImGuiDockNodeFlags_NoDockingSplit, "// Disable other windows/nodes from splitting this node.")
    .value("NoResize", ImGuiDockNodeFlags_NoResize, "Saved // Disable resizing node using the splitter/separators. Useful with programmatically setup dockspaces.")
    .value("AutoHideTabBar", ImGuiDockNodeFlags_AutoHideTabBar, "// Tab bar will automatically hide when there is a single window in the dock node.")
    .value("NoUndocking", ImGuiDockNodeFlags_NoUndocking, "// Disable undocking this node.")
    .value("NoSplit", ImGuiDockNodeFlags_NoSplit, "Renamed in 1.90")
    .value("NoDockingInCentralNode", ImGuiDockNodeFlags_NoDockingInCentralNode, "Renamed in 1.90")
  ;
  ...
  py::enum_<ImGuiDragDropFlags_>(m, "DragDropFlags", py::arithmetic())
    ...
    .value("PayloadAutoExpire", ImGuiDragDropFlags_PayloadAutoExpire, "Automatically expire the payload if the source cease to be submitted (otherwise payloads are persisting while being dragged)")
    .value("PayloadNoCrossContext", ImGuiDragDropFlags_PayloadNoCrossContext, "Hint to specify that the payload may not be copied outside current dear imgui context.")
    .value("PayloadNoCrossProcess", ImGuiDragDropFlags_PayloadNoCrossProcess, "Hint to specify that the payload may not be copied outside current process.")
    ...
    .value("AcceptDrawAsHovered", ImGuiDragDropFlags_AcceptDrawAsHovered, "Accepting item will render as if hovered. Useful for e.g. a Button() used as a drop target.")
    .value("AcceptPeekOnly", ImGuiDragDropFlags_AcceptPeekOnly, "For peeking ahead and inspecting the payload before delivery.")
    .value("SourceAutoExpirePayload", ImGuiDragDropFlags_SourceAutoExpirePayload, "Renamed in 1.90.9")
  ;
  ...
  py::enum_<ImGuiInputFlags_>(m, "InputFlags", py::arithmetic())
    ...
    .value("RouteActive", ImGuiInputFlags_RouteActive, "Route to active item only.")
    .value("RouteFocused", ImGuiInputFlags_RouteFocused, "Route to windows in the focus stack (DEFAULT). Deep-most focused window takes inputs. Active item takes inputs over deep-most focused window.")
    .value("RouteGlobal", ImGuiInputFlags_RouteGlobal, "Global route (unless a focused window or active item registered the route).")
    .value("RouteAlways", ImGuiInputFlags_RouteAlways, "Do not register route, poll keys directly.")
    .value("RouteOverFocused", ImGuiInputFlags_RouteOverFocused, "Option: global route: higher priority than focused route (unless active item in focused route).")
    ...
    .value("RouteUnlessBgFocused", ImGuiInputFlags_RouteUnlessBgFocused, "Option: global route: will not be applied if underlying background/void is focused (== no Dear ImGui windows are focused). Useful for overlay applications.")
    .value("RouteFromRootWindow", ImGuiInputFlags_RouteFromRootWindow, "Option: route evaluated from the point of view of root window rather than current window.")
  ...
  py::enum_<ImGuiConfigFlags_>(m, "ConfigFlags", py::arithmetic())
    ...
    .value("NoMouse", ImGuiConfigFlags_NoMouse, "Instruct dear imgui to disable mouse inputs and interactions.")
    .value("NoMouseCursorChange", ImGuiConfigFlags_NoMouseCursorChange, "Instruct backend to not alter mouse cursor shape and visibility. Use if the backend cursor changes are interfering with yours and you don't want to use SetMouseCursor() to change mouse cursor. You may want to honor requests from imgui by reading GetMouseCursor() yourself instead.")
    .value("NoKeyboard", ImGuiConfigFlags_NoKeyboard, "Instruct dear imgui to disable keyboard inputs and interactions. This is done by ignoring keyboard events and clearing existing states.")
    ...
    .value("NavEnableSetMousePos", ImGuiConfigFlags_NavEnableSetMousePos, "[moved/renamed in 1.91.4] -> use bool io.ConfigNavMoveSetMousePos")
    .value("NavNoCaptureKeyboard", ImGuiConfigFlags_NavNoCaptureKeyboard, "[moved/renamed in 1.91.4] -> use bool io.ConfigNavCaptureKeyboard")
    .value("DpiEnableScaleFonts", ImGuiConfigFlags_DpiEnableScaleFonts, "[moved/renamed in 1.92.0] -> use bool io.ConfigDpiScaleFonts")
    .value("DpiEnableScaleViewports", ImGuiConfigFlags_DpiEnableScaleViewports, "[moved/renamed in 1.92.0] -> use bool io.ConfigDpiScaleViewports")
  ;
  ...
  py::enum_<ImGuiBackendFlags_>(m, "BackendFlags", py::arithmetic())
    ...
    .value("HasSetMousePos", ImGuiBackendFlags_HasSetMousePos, "Backend Platform supports io.WantSetMousePos requests to reposition the OS mouse position (only used if io.ConfigNavMoveSetMousePos is set).")
    ...
    .value("RendererHasTextures", ImGuiBackendFlags_RendererHasTextures, "Backend Renderer supports ImTextureData requests to create/update/destroy textures. This enables incremental texture updates and texture reloads. See https://github.com/ocornut/imgui/blob/master/docs/BACKENDS.md for instructions on how to upgrade your custom backend.")
    ...
    .value("HasParentViewport", ImGuiBackendFlags_HasParentViewport, "Backend Platform supports honoring viewport->ParentViewport/ParentViewportId value, by applying the corresponding parent/child relation at the Platform level.")
  ;
  ...
  py::enum_<ImGuiCol_>(m, "Col", py::arithmetic())
    ...
    .value("TitleBg", ImGuiCol_TitleBg, "Title bar")
    .value("TitleBgActive", ImGuiCol_TitleBgActive, "Title bar when focused")
    .value("TitleBgCollapsed", ImGuiCol_TitleBgCollapsed, "Title bar when collapsed")
    ...
    .value("CheckMark", ImGuiCol_CheckMark, "Checkbox tick and RadioButton circle")
    ...
    .value("InputTextCursor", ImGuiCol_InputTextCursor, "InputText cursor/caret")
    .value("TabHovered", ImGuiCol_TabHovered, "Tab background, when hovered")
    .value("Tab", ImGuiCol_Tab, "Tab background, when tab-bar is focused & tab is unselected")
    .value("TabSelected", ImGuiCol_TabSelected, "Tab background, when tab-bar is focused & tab is selected")
    .value("TabSelectedOverline", ImGuiCol_TabSelectedOverline, "Tab horizontal overline, when tab-bar is focused & tab is selected")
    .value("TabDimmed", ImGuiCol_TabDimmed, "Tab background, when tab-bar is unfocused & tab is unselected")
    .value("TabDimmedSelected", ImGuiCol_TabDimmedSelected, "Tab background, when tab-bar is unfocused & tab is selected")
    .value("TabDimmedSelectedOverline", ImGuiCol_TabDimmedSelectedOverline, "..horizontal overline, when tab-bar is unfocused & tab is selected")
    ...
    .value("TextLink", ImGuiCol_TextLink, "Hyperlink color")
    .value("TextSelectedBg", ImGuiCol_TextSelectedBg, "Selected text inside an InputText")
    .value("TreeLines", ImGuiCol_TreeLines, "Tree node hierarchy outlines when using ImGuiTreeNodeFlags_DrawLines")
    .value("DragDropTarget", ImGuiCol_DragDropTarget, "Rectangle border highlighting a drop target")
    .value("DragDropTargetBg", ImGuiCol_DragDropTargetBg, "Rectangle background highlighting a drop target")
    .value("UnsavedMarker", ImGuiCol_UnsavedMarker, "Unsaved Document marker (in window title and tabs)")
    .value("NavCursor", ImGuiCol_NavCursor, "Color of keyboard/gamepad navigation cursor/rectangle, when visible")
    ...
    .value("TabActive", ImGuiCol_TabActive, "[renamed in 1.90.9]")
    .value("TabUnfocused", ImGuiCol_TabUnfocused, "[renamed in 1.90.9]")
    .value("TabUnfocusedActive", ImGuiCol_TabUnfocusedActive, "[renamed in 1.90.9]")
    .value("NavHighlight", ImGuiCol_NavHighlight, "[renamed in 1.91.4]")
  ;
  ...
  py::enum_<ImGuiStyleVar_>(m, "StyleVar", py::arithmetic())
    ...
    .value("ScrollbarPadding", ImGuiStyleVar_ScrollbarPadding, "float     ScrollbarPadding")
    ...
    .value("ImageRounding", ImGuiStyleVar_ImageRounding, "float     ImageRounding")
    .value("ImageBorderSize", ImGuiStyleVar_ImageBorderSize, "float     ImageBorderSize")
    .value("TabRounding", ImGuiStyleVar_TabRounding, "float     TabRounding")
    .value("TabBorderSize", ImGuiStyleVar_TabBorderSize, "float     TabBorderSize")
    .value("TabMinWidthBase", ImGuiStyleVar_TabMinWidthBase, "float     TabMinWidthBase")
    .value("TabMinWidthShrink", ImGuiStyleVar_TabMinWidthShrink, "float     TabMinWidthShrink")
    .value("TabBarBorderSize", ImGuiStyleVar_TabBarBorderSize, "float     TabBarBorderSize")
    .value("TabBarOverlineSize", ImGuiStyleVar_TabBarOverlineSize, "float     TabBarOverlineSize")
    .value("TableAngledHeadersAngle", ImGuiStyleVar_TableAngledHeadersAngle, "float     TableAngledHeadersAngle")
    .value("TableAngledHeadersTextAlign", ImGuiStyleVar_TableAngledHeadersTextAlign, "ImVec2  TableAngledHeadersTextAlign")
    .value("TreeLinesSize", ImGuiStyleVar_TreeLinesSize, "float     TreeLinesSize")
    .value("TreeLinesRounding", ImGuiStyleVar_TreeLinesRounding, "float     TreeLinesRounding")
  ...
  py::enum_<ImGuiButtonFlags_>(m, "ButtonFlags", py::arithmetic())
    ...
    .value("MouseButtonMask_", ImGuiButtonFlags_MouseButtonMask_, "[Internal]")
    .value("EnableNav", ImGuiButtonFlags_EnableNav, "InvisibleButton(): do not disable navigation/tabbing. Otherwise disabled by default.")
  ;
  ...
  py::enum_<ImGuiColorEditFlags_>(m, "ColorEditFlags", py::arithmetic())
    ...
    .value("NoDragDrop", ImGuiColorEditFlags_NoDragDrop, "// ColorEdit: disable drag and drop target/source. ColorButton: disable drag and drop source. ")
    .value("NoBorder", ImGuiColorEditFlags_NoBorder, "// ColorButton: disable border (which is enforced by default)")
    .value("NoColorMarkers", ImGuiColorEditFlags_NoColorMarkers, "// ColorEdit: disable rendering R/G/B/A color marker. May also be disabled globally by setting style.ColorMarkerSize = 0.")
    .value("AlphaOpaque", ImGuiColorEditFlags_AlphaOpaque, "// ColorEdit, ColorPicker, ColorButton: disable alpha in the preview,. Contrary to _NoAlpha it may still be edited when calling ColorEdit4()/ColorPicker4(). For ColorButton() this does the same as _NoAlpha.")
    .value("AlphaNoBg", ImGuiColorEditFlags_AlphaNoBg, "// ColorEdit, ColorPicker, ColorButton: disable rendering a checkerboard background behind transparent color.")
    .value("AlphaPreviewHalf", ImGuiColorEditFlags_AlphaPreviewHalf, "// ColorEdit, ColorPicker, ColorButton: display half opaque / half transparent preview.")
    ...
    .value("AlphaMask_", ImGuiColorEditFlags_AlphaMask_)
    ...
    .value("AlphaPreview", ImGuiColorEditFlags_AlphaPreview, "Removed in 1.91.8. This is the default now. Will display a checkerboard unless ImGuiColorEditFlags_AlphaNoBg is set.")
  ;
  ...
  py::enum_<ImGuiSliderFlags_>(m, "SliderFlags", py::arithmetic())
    ...
    .value("WrapAround", ImGuiSliderFlags_WrapAround, "Enable wrapping around from max to min and from min to max. Only supported by DragXXX() functions for now.")
    .value("ClampOnInput", ImGuiSliderFlags_ClampOnInput, "Clamp value to min/max bounds when input manually with Ctrl+Click. By default Ctrl+Click allows going out of bounds.")
    .value("ClampZeroRange", ImGuiSliderFlags_ClampZeroRange, "Clamp even if min==max==0.0f. Otherwise due to legacy reason DragXXX functions don't clamp with those values. When your clamping limits are dynamic you almost always want to use it.")
    .value("NoSpeedTweaks", ImGuiSliderFlags_NoSpeedTweaks, "Disable keyboard modifiers altering tweak speed. Useful if you want to alter tweak speed yourself based on your own logic.")
    .value("ColorMarkers", ImGuiSliderFlags_ColorMarkers, "DragScalarN(), SliderScalarN(): Draw R/G/B/A color markers on each component.")
    .value("AlwaysClamp", ImGuiSliderFlags_AlwaysClamp)
    .value("InvalidMask_", ImGuiSliderFlags_InvalidMask_, "[Internal] We treat using those bits as being potentially a 'float power' argument from legacy API (obsoleted 2020-08) that has got miscast to this enum, and will trigger an assert if needed.")
  ;
  ...
  py::enum_<ImGuiTableFlags_>(m, "TableFlags", py::arithmetic())
    ...
    .value("NoSavedSettings", ImGuiTableFlags_NoSavedSettings, "Disable persisting columns order, width, visibility and sort settings in the .ini file.")
    ...
    .value("HighlightHoveredColumn", ImGuiTableFlags_HighlightHoveredColumn, "Highlight column headers when hovered (may evolve into a fuller highlight)")
  ...
  py::enum_<ImGuiTableColumnFlags_>(m, "TableColumnFlags", py::arithmetic())
    ...
    .value("AngledHeader", ImGuiTableColumnFlags_AngledHeader, "TableHeadersRow() will submit an angled header row for this column. Note this will add an extra row.")
  ...
  py::enum_<ImGuiListClipperFlags_>(m, "ListClipperFlags", py::arithmetic())
    .value("NONE", ImGuiListClipperFlags_None)
    .value("NoSetTableRowCounters", ImGuiListClipperFlags_NoSetTableRowCounters, "[Internal] Disabled modifying table row counters. Avoid assumption that 1 clipper item == 1 table row.")
  ;

  py::enum_<ImGuiMultiSelectFlags_>(m, "MultiSelectFlags", py::arithmetic())
    .value("NONE", ImGuiMultiSelectFlags_None)
    .value("SingleSelect", ImGuiMultiSelectFlags_SingleSelect, "Disable selecting more than one item. This is available to allow single-selection code to share same code/logic if desired. It essentially disables the main purpose of BeginMultiSelect() tho!")
    .value("NoSelectAll", ImGuiMultiSelectFlags_NoSelectAll, "Disable Ctrl+A shortcut to select all.")
    .value("NoRangeSelect", ImGuiMultiSelectFlags_NoRangeSelect, "Disable Shift+selection mouse/keyboard support (useful for unordered 2D selection). With BoxSelect is also ensure contiguous SetRange requests are not combined into one. This allows not handling interpolation in SetRange requests.")
    .value("NoAutoSelect", ImGuiMultiSelectFlags_NoAutoSelect, "Disable selecting items when navigating (useful for e.g. supporting range-select in a list of checkboxes).")
    .value("NoAutoClear", ImGuiMultiSelectFlags_NoAutoClear, "Disable clearing selection when navigating or selecting another one (generally used with ImGuiMultiSelectFlags_NoAutoSelect. useful for e.g. supporting range-select in a list of checkboxes).")
    .value("NoAutoClearOnReselect", ImGuiMultiSelectFlags_NoAutoClearOnReselect, "Disable clearing selection when clicking/selecting an already selected item.")
    .value("BoxSelect1d", ImGuiMultiSelectFlags_BoxSelect1d, "Enable box-selection with same width and same x pos items (e.g. full row Selectable()). Box-selection works better with little bit of spacing between items hit-box in order to be able to aim at empty space.")
    .value("BoxSelect2d", ImGuiMultiSelectFlags_BoxSelect2d, "Enable box-selection with varying width or varying x pos items support (e.g. different width labels, or 2D layout/grid). This is slower: alters clipping logic so that e.g. horizontal movements will update selection of normally clipped items.")
    .value("BoxSelectNoScroll", ImGuiMultiSelectFlags_BoxSelectNoScroll, "Disable scrolling when box-selecting near edges of scope.")
    .value("ClearOnEscape", ImGuiMultiSelectFlags_ClearOnEscape, "Clear selection when pressing Escape while scope is focused.")
    .value("ClearOnClickVoid", ImGuiMultiSelectFlags_ClearOnClickVoid, "Clear selection when clicking on empty location within scope.")
    .value("ScopeWindow", ImGuiMultiSelectFlags_ScopeWindow, "Scope for _BoxSelect and _ClearOnClickVoid is whole window (Default). Use if BeginMultiSelect() covers a whole window or used a single time in same window.")
    .value("ScopeRect", ImGuiMultiSelectFlags_ScopeRect, "Scope for _BoxSelect and _ClearOnClickVoid is rectangle encompassing BeginMultiSelect()/EndMultiSelect(). Use if BeginMultiSelect() is called multiple times in same window.")
    .value("SelectOnClick", ImGuiMultiSelectFlags_SelectOnClick, "Apply selection on mouse down when clicking on unselected item. (Default)")
    .value("SelectOnClickRelease", ImGuiMultiSelectFlags_SelectOnClickRelease, "Apply selection on mouse release when clicking an unselected item. Allow dragging an unselected item without altering selection.")
    .value("NavWrapX", ImGuiMultiSelectFlags_NavWrapX, "[Temporary] Enable navigation wrapping on X axis. Provided as a convenience because we don't have a design for the general Nav API for this yet. When the more general feature be public we may obsolete this flag in favor of new one.")
    .value("NoSelectOnRightClick", ImGuiMultiSelectFlags_NoSelectOnRightClick, "Disable default right-click processing, which selects item on mouse down, and is designed for context-menus.")
  ;
  ...
  py::enum_<ImGuiViewportFlags_>(m, "ViewportFlags", py::arithmetic())
    ...
    .value("OwnedByApp", ImGuiViewportFlags_OwnedByApp, "Platform Window: Is created/managed by the user application? (rather than our backend)")
  ...
  m.def("End", py::overload_cast<>(&ImGui::End));
  m.def("End", py::overload_cast<>(&ImGui::End), "Automatically called on the last call of Step() that returns false.");
  m.def("BeginChild", py::overload_cast<ImGuiID, const ImVec2&, ImGuiChildFlags, ImGuiWindowFlags>(&ImGui::BeginChild), py::arg("id"), py::arg("size") = ImVec2(0, 0), py::arg("child_flags") = 0, py::arg("window_flags") = 0);
  m.def("BeginChild", py::overload_cast<const char*, const ImVec2&, ImGuiChildFlags, ImGuiWindowFlags>(&ImGui::BeginChild), py::arg("str_id"), py::arg("size") = ImVec2(0, 0), py::arg("child_flags") = 0, py::arg("window_flags") = 0);
  ...
  m.def("IsWindowHovered", &ImGui::IsWindowHovered, py::arg("flags") = 0, "is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See ImGuiHoveredFlags_ for options. IMPORTANT: If you are trying to check whether your mouse should be dispatched to Dear ImGui or to your underlying app, you should not use this function! Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry \"How can I tell whether to dispatch mouse/keyboard to Dear ImGui or my application?\" for details.");
  m.def("GetWindowPos", &ImGui::GetWindowPos, "get current window position in screen space (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)");
  m.def("GetWindowSize", &ImGui::GetWindowSize, "get current window size (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)");
  m.def("GetWindowWidth", &ImGui::GetWindowWidth, "get current window width (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for GetWindowSize().x.");
  m.def("GetWindowHeight", &ImGui::GetWindowHeight, "get current window height (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for GetWindowSize().y.");
  ...
  m.def("SetWindowFontScale", &ImGui::SetWindowFontScale, py::arg("scale"), "Set font scale factor for current window. Prefer using PushFont(NULL, style.FontSizeBase * factor) or use style.FontScaleMain to scale all windows.");
  m.def("GetContentRegionAvail", &ImGui::GetContentRegionAvail, "available space from current position. THIS IS YOUR BEST FRIEND.");
  m.def("GetContentRegionMax", &ImGui::GetContentRegionMax, "Content boundaries max (e.g. window boundaries including scrolling, or current column boundaries). You should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!");
  m.def("GetWindowContentRegionMin", &ImGui::GetWindowContentRegionMin, "Content boundaries min for the window (roughly (0,0)-Scroll), in window-local coordinates. You should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!");
  m.def("GetWindowContentRegionMax", &ImGui::GetWindowContentRegionMax, "Content boundaries max for the window (roughly (0,0)+Size-Scroll), in window-local coordinates. You should never need this. Always use GetCursorScreenPos() and GetContentRegionAvail()!");
  ...
  m.def("PushStyleVar", py::overload_cast<ImGuiStyleVar, const ImVec2&>(&ImGui::PushStyleVar), py::arg("idx"), py::arg("val"), "modify a style ImVec2 variable. \"");
  ...
  m.def("GetCursorPos", &ImGui::GetCursorPos, "[window-local] cursor position in window-local coordinates. This is not your best friend.");
  m.def("GetCursorPosX", &ImGui::GetCursorPosX, "[window-local] \"");
  m.def("GetCursorPosY", &ImGui::GetCursorPosY, "[window-local] \"");
  m.def("SetCursorPos", &ImGui::SetCursorPos, py::arg("local_pos"), "[window-local] \"");
  m.def("SetCursorPosX", &ImGui::SetCursorPosX, py::arg("local_x"), "[window-local] \"");
  m.def("SetCursorPosY", &ImGui::SetCursorPosY, py::arg("local_y"), "[window-local] \"");
  m.def("GetCursorStartPos", &ImGui::GetCursorStartPos, "[window-local] initial cursor position, in window-local coordinates. Call GetCursorScreenPos() after Begin() to get the absolute coordinates version.");
  m.def("GetCursorScreenPos", &ImGui::GetCursorScreenPos, "cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND (prefer using this rather than GetCursorPos(), also more useful to work with ImDrawList API).");
  m.def("SetCursorScreenPos", &ImGui::SetCursorScreenPos, py::arg("pos"), "cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND.");
  ...
  m.def("SmallButton", &ImGui::SmallButton, py::arg("label"), "button with (FramePadding.y == 0) to easily embed within text");
  ...
  m.def("OpenPopupOnItemClick", &ImGui::OpenPopupOnItemClick, py::arg("str_id") = NULL, py::arg("popup_flags") = 0, "helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)");
  m.def("CloseCurrentPopup", &ImGui::CloseCurrentPopup, "manually close the popup we have begin-ed into.");
  m.def("BeginPopupContextItem", &ImGui::BeginPopupContextItem, py::arg("str_id") = NULL, py::arg("popup_flags") = 0, "open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!");
  m.def("BeginPopupContextWindow", &ImGui::BeginPopupContextWindow, py::arg("str_id") = NULL, py::arg("popup_flags") = 0, "open+begin popup when clicked on current window.");
  m.def("BeginPopupContextVoid", &ImGui::BeginPopupContextVoid, py::arg("str_id") = NULL, py::arg("popup_flags") = 0, "open+begin popup when clicked in void (where there are no windows).");
  m.def("IsPopupOpen", &ImGui::IsPopupOpen, py::arg("str_id"), py::arg("flags") = 0, "return true if the popup is open.");
  m.def("BeginTable", &ImGui::BeginTable, py::arg("str_id"), py::arg("columns"), py::arg("flags") = 0, py::arg("outer_size") = ImVec2(0.0f, 0.0f), py::arg("inner_width") = 0.0f);
  m.def("EndTable", &ImGui::EndTable, "only call EndTable() if BeginTable() returns true!");
  m.def("TableNextRow", &ImGui::TableNextRow, py::arg("row_flags") = 0, py::arg("min_row_height") = 0.0f, "append into the first cell of a new row. 'min_row_height' include the minimum top and bottom padding aka CellPadding.y * 2.0f.");
  ...
  m.def("TableHeadersRow", &ImGui::TableHeadersRow, "submit a row with headers cells based on data provided to TableSetupColumn() + submit context menu");
  ...
  m.def("TableGetRowIndex", &ImGui::TableGetRowIndex, "return current row index (header rows are accounted for)");
  ...
  m.def("SetItemDefaultFocus", &ImGui::SetItemDefaultFocus, "make last item the default focused item of a newly appearing window.");
  ...
  m.def("IsKeyPressed", &ImGui::IsKeyPressed, py::arg("key"), py::arg("repeat") = true, "was key pressed (went from !Down to Down)? Repeat rate uses io.KeyRepeatDelay / KeyRepeatRate.");
  ...
  m.def("IsMouseDragging", &ImGui::IsMouseDragging, py::arg("button"), py::arg("lock_threshold") = -1.0f, "is mouse dragging? (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)");
  ...
  m.def("SetNextFrameWantCaptureMouse", &ImGui::SetNextFrameWantCaptureMouse, py::arg("want_capture_mouse"), "Override io.WantCaptureMouse flag next frame (said flag is left for your application to handle, typical when true it instructs your app to ignore inputs). This is equivalent to setting \"io.WantCaptureMouse = want_capture_mouse;\" after the next NewFrame() call.");
  ...
  m.def("Selectable", [](char const* label, bool selected, ImGuiSelectableFlags flags, ImVec2 const& size) {
    bool ret = ImGui::Selectable(label, &selected, flags, size);
    return py::make_tuple(ret, selected);
  }, py::arg("label"), py::arg("selected"), py::arg("flags") = 0, py::arg("size") = ImVec2(0,0));

  m.def("Selectable", [](char const* label, bool selected, ImGuiSelectableFlags flags, ImVec2 const& size) {
    return ImGui::Selectable(label, selected, flags, size);
  }, py::arg("label"), py::arg("selected") = false, py::arg("flags") = 0, py::arg("size") = ImVec2(0,0));

  m.def("MenuItem", [](char const* label, char const* shortcut, bool selected, bool enabled) {
    bool ret = ImGui::MenuItem(label, shortcut, &selected, enabled);
    return py::make_tuple(ret, selected);
  }, py::arg("label"), py::arg("shortcut"), py::arg("selected"), py::arg("enabled") = true);

  m.def("MenuItem", [](char const* label, char const* shortcut, bool selected, bool enabled) {
    return ImGui::MenuItem(label, shortcut, selected, enabled);
  }, py::arg("label"), py::arg("shortcut") = (const char*)0, py::arg("selected") = false, py::arg("enabled") = true);

  m.def("CollapsingHeader", [](char const* label, bool visible, ImGuiTreeNodeFlags flags) {
    bool ret = ImGui::CollapsingHeader(label, &visible, flags);
    return py::make_tuple(ret, visible);
  }, py::arg("label"), py::arg("visible"), py::arg("flags") = 0);

  m.def("CollapsingHeader", [](char const* label, ImGuiTreeNodeFlags flags) {
    return ImGui::CollapsingHeader(label, flags);
  }, py::arg("label"), py::arg("flags") = 0);

  m.def("DragFloat", [](const char* label, float v, float v_speed, float v_min, float v_max, const char* format, ImGuiSliderFlags flags) {
  ...
  m.def("DragInt", [](const char* label, int v, float v_speed, int v_min, int v_max, const char* format, ImGuiSliderFlags flags) {
  ...
  m.def("SliderFloat", [](const char* label, float v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags) {
  ...
  m.def("SliderInt", [](const char* label, int v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags) {
  ...
  m.def("ColorEdit3", [](const char* label, py::object col, ImGuiColorEditFlags flags) {
      float c[3] = {0.f, 0.f, 0.f};
      if (py::isinstance<py::tuple>(col) || py::isinstance<py::list>(col)) {
          py::sequence t = col;
          if (t.size() >= 3) { 
              c[0] = t[0].cast<float>(); 
              c[1] = t[1].cast<float>(); 
      ...
      bool ret = ImGui::ColorEdit3(label, c, flags);
      return py::make_tuple(ret, py::make_tuple(c[0], c[1], c[2]));
  }, py::arg("label"), py::arg("col"), py::arg("flags")=0);

  m.def("ColorEdit4", [](const char* label, py::object col, ImGuiColorEditFlags flags) {
      ...
      bool ret = ImGui::ColorEdit4(label, c, flags);
      return py::make_tuple(ret, py::make_tuple(c[0], c[1], c[2], c[3]));
  }, py::arg("label"), py::arg("col"), py::arg("flags")=0);

  m.def("Combo", [](const char* label, int current_item, const std::vector<std::string>& items, int popup_max_height_in_items) -> py::tuple {
      if (items.empty()) return py::make_tuple(false, current_item);
      std::vector<const char*> c_items;
      c_items.reserve(items.size());
      for (const auto& item : items) c_items.push_back(item.c_str());
      bool ret = ImGui::Combo(label, &current_item, c_items.data(), static_cast<int>(c_items.size()), popup_max_height_in_items);
      return py::make_tuple(ret, current_item);
  }, py::arg("label"), py::arg("current_item"), py::arg("items"), py::arg("popup_max_height_in_items")=-1);

  m.def("ListBox", [](const char* label, int current_item, const std::vector<std::string>& items, int height_in_items) -> py::tuple {
      if (items.empty()) return py::make_tuple(false, current_item);
      std::vector<const char*> c_items;
      c_items.reserve(items.size());
      for (const auto& item : items) c_items.push_back(item.c_str());
      bool ret = ImGui::ListBox(label, &current_item, c_items.data(), static_cast<int>(c_items.size()), height_in_items);
      return py::make_tuple(ret, current_item);
  }, py::arg("label"), py::arg("current_item"), py::arg("items"), py::arg("height_in_items")=-1);
  
  m.def("DragScalar", [](char const* label, ImGuiDataType type, py::object value, float speed, py::object vmin, py::object vmax, char const* format, ImGuiSliderFlags flags) -> py::tuple {
...
#undef TYPE_CASE
  ...
  m.def("SliderScalar", [](char const* label, ImGuiDataType type, py::object value, py::object vmin, py::object vmax, char const* format, ImGuiSliderFlags flags) -> py::tuple {
    int8_t   i8[4]  = {0}, i8minmax[2];
    uint8_t  u8[4]  = {0}, u8minmax[2];
    int16_t  i16[4] = {0}, i16minmax[2];
    uint16_t u16[4] = {0}, u16minmax[2];
